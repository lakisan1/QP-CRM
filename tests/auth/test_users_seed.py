...
def test_seeded_accounts_have_no_forced_change_flag():
    # The forced change-on-first-login step is RETIRED (passwords are set
    # by the admin in Admin -> Users), so seeding leaves the legacy
    # must_change_password column at 0 for every account.
    import sqlite3
    ...
    users = _users(conn)
    assert all(u["must_change_password"] == 0 for u in users.values())
    conn.close()
