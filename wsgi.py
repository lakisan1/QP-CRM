...
WSGI entrypoint for QP-CRM under a production WSGI server (gunicorn).
    ...
    gunicorn --preload --bind 0.0.0.0:5000 wsgi:application
...
rent/app.py:251). All connections come from shared.db.get_db(), which opens
...
# Make the project root (shared/, pricing/, offer/, ...) importable no matter
# which directory gunicorn is started from. main.py does the same for its own
# runs (main.py:5-7).
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
if CURRENT_DIR not in sys.path:
    sys.path.insert(0, CURRENT_DIR)

from shared.config import APP_DATA_DIR, IMAGE_DIR, APP_ASSETS_DIR
...
# 2) Run the DB init/migration sequence in the same order as `python main.py`
...
from pricing.app import init_db as pricing_init_db, migrate_schema as pricing_migrate_schema  # noqa: E402
from offer.app import init_db as offer_init_db  # noqa: E402
from admin.app import init_db as admin_init_db  # noqa: E402
from rent.app import init_db as rent_init_db  # noqa: E402
...
# 3) The WSGI application object gunicorn serves (`wsgi:application`):
#    the DispatcherMiddleware merging the landing app with the six sub-apps
#    (main.py:52-59). Importing `main` at this point also loads the remaining
#    sub-apps (sale, settings, pricing.api_v1) exactly as a normal
#    `python main.py` run would; the four modules above are already in
#    sys.modules, so their module-level code is not executed twice.
from main import application  # noqa: E402, F401
